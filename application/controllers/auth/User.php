...
class User extends CI_Controller
{
  ...
  public function tambah()
  {
    ...
    if ($valid->run()) {
      ...
      if (!$this->upload->do_upload('foto_user')) {
        ...
        redirect(base_url('auth/user'), 'refresh');
      }
    }
    //End Masuk Database
    $data = array(
  ...
  public function edit($id_user)
  {
    ...
    $valid->set_rules(
      ...
      'Anda',
      'required',
      array('required'      => '%s Tidak dapat mengubah data auth')
    );
    ...
    if ($valid->run()) {
      ...
      if (!empty($_FILES['foto_user']['name'])) {
        ...
        if (!$this->upload->do_upload('foto_user')) {
          ...
          //End Validasi
          $data = array(
            'title'  => 'Edit User authistrator: ' . $user->nama,
            'user'   => $user,
            'error_upload' => $this->upload->display_errors(),
            'isi'    => 'auth/user/edit'
          );
...
/* Location /application/controller/auth/User.php */
